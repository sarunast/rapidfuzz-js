...
interface Scorers {
  readonly distance: PreparedCapability &
    ((s1: Sequence, s2: Sequence, options?: { scoreCutoff?: number }) => number)
  readonly similarity: PreparedCapability &
    ((s1: Sequence, s2: Sequence, options?: { scoreCutoff?: number }) => number)
  readonly normalizedDistance: PreparedCapability &
    ((s1: MaybeSequence, s2: MaybeSequence, options?: { scoreCutoff?: number }) => number)
  readonly normalizedSimilarity: PreparedCapability &
    ((s1: MaybeSequence, s2: MaybeSequence, options?: { scoreCutoff?: number }) => number)
}
    ...
    it(`for ${name}, at every cutoff of every convention`, () => {
      for (const convention of CONVENTIONS) {
        ...
        for (const [s1, s2] of PAIRS) {
          ...
          expect(score(s2, null), what).toBeCloseTo(scorer(s1, s2), 9)

          for (const scoreCutoff of cutoffs) {
            expect(score(s2, scoreCutoff), `${what} at ${scoreCutoff}`).toBeCloseTo(
...
describe('what only a prepared scorer is asked', () => {
  for (const [name, scorers] of FAMILIES) {
    it(`reports maximum dissimilarity for a missing choice — ${name}`, () => {
      expect(preparedOf(scorers.normalizedDistance)('abcd', {})(null, null)).toBe(1)
      expect(preparedOf(scorers.normalizedSimilarity)('abcd', {})(null, null)).toBe(0)
      expect(preparedOf(scorers.normalizedDistance)('abcd', {})(Number.NaN, null)).toBe(1)
    })
    ...
    it(`refuses a missing choice for a raw convention — ${name}`, () => {
      expect(() => preparedOf(scorers.distance)('abcd', {})(null, null)).toThrow(
        TypeError,
      )
      expect(() => preparedOf(scorers.similarity)('abcd', {})(null, null)).toThrow(
        TypeError,
      )
    })
...
describe('a prepared Hamming scorer carries its pad option', () => {
  it('refuses a length mismatch when padding is off', () => {
    ...
    expect(score('abcd', null)).toBe(0)
    expect(() => score('abcde', null)).toThrow('Sequences are not the same length.')
  })

  it('pads by default', () => {
    expect(preparedOf(hammingDistance)('abcd', {})('abcde', null)).toBe(1)
  })
})
