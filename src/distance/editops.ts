/**
 * Port of `rapidfuzz.distance._initialize` (see `_initialize_py.py`) — the
...
import { hasSurrogatePair } from '../_common.js'
 ...
 *
 * - `replace`: `src[srcPos]` should be replaced by `dest[destPos]`
 * - `delete`: `src[srcPos]` should be deleted
 * - `insert`: `dest[destPos]` should be inserted at `src[srcPos]`
 */
...
/**
 * A block edit operation, describing the same alignment in runs. Carries the
 * same information as the opcodes of Python's `difflib.SequenceMatcher`.
 */
...
/**
 * A matching subsequence.
 *
 * Upstream spells these `(a, b, size)`, positionally. Named here for the same
 * two strings every other type in this file names, since nothing unpacks them
 * any more.
 */
 ...
 * modules and left out of both barrels, so nothing outside `src/distance/` can
 * name them.
 *
...
export let opcodesFromValidated: (
 ...
 *
 * Readonly on the same terms as {@link Editops}.
 */
export class Opcodes {
  ...
  /** See {@link Editops.fromOperations}. Adjacent blocks with one tag are merged. */
  static fromOperations(
    operations: readonly Opcode[],
    srcLen: number,
    destLen: number,
   ...
   * Apply these operations to `source`, drawing replacements from `destination`.
   *
   * Every block is a run, so both sides are sliced and both are worth a view:
   * a BMP string skips `Array.from` and the `join` that undid it, which
   * measured 5-9x faster on edit-dense scripts and far more on sparse ones.
   */
  apply(source: string, destination: string): string {
